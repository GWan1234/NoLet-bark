...
class ImageManager {
    ...
    class func downloadImage(_ imageUrl: String,  expiration: StorageExpiration = .never) async -> String? {
        ...
        if proxyServer.status,
           let fileUrl = try? await downloadFile(from: imageUrl, proxy: proxyServer.url) {
    ...
    class func downloadFile(from mediaUrl: String, proxy proxyServer: String? = nil) async throws -> URL {
        

        guard let fileURL = URL(string: proxyServer ?? mediaUrl) else {
            throw "invalidURL"
        }
